...
def get_forward_backward_func():
    ...
    """
    try:
        print_datetime("Starting get_forward_backward_func")
        print_datetime(f"Pipeline parallel size: {parallel_state.get_pipeline_model_parallel_world_size()}")
        print_datetime(f"Virtual pipeline parallel size: {parallel_state.get_virtual_pipeline_model_parallel_world_size()}")
        print_datetime(f"Pipeline parallel rank: {parallel_state.get_pipeline_model_parallel_rank()}")
    
        print_datetime("Get parallel_size")
        pipeline_model_parallel_size = parallel_state.get_pipeline_model_parallel_world_size()
        print_datetime(f"Parallel Size: {pipeline_model_parallel_size}")
        if pipeline_model_parallel_size > 1:
            if parallel_state.get_virtual_pipeline_model_parallel_world_size() is not None:
                print_datetime("Using interleaved pipelining")
                forward_backward_func = forward_backward_pipelining_with_interleaving
            else:
                print_datetime("Not using interleaved pipelining")
                forward_backward_func = forward_backward_pipelining_without_interleaving
        else:
            print_datetime("forward backward with no pipelining")
            forward_backward_func = forward_backward_no_pipelining
        print_datetime("End of Forward Backward Function")
        return forward_backward_func
    except Exception as e:
        print(f"Error in get_forward_backward_func: {str(e)}")
        raise
...
def forward_step(
    ...
    print_datetime(f"Forward Step Function Start")
    if config.timers is not None:
        config.timers('forward-compute', log_level=2).start()

    print_datetime(f"Set first microbatch")
    ...
    print_datetime(f"Get attribute wrapped model")
    ...
    print_datetime(f"getting data from Loss function")
    ...
    print_datetime(f"Forward Step Function Done")
    return [output_tensor], num_tokens
...
def recv_forward(tensor_shapes, config):
    print_datetime("recv_forward start")
    input_tensors = []
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    try:
        print_datetime("starting forward_backward_pipelining_without_interleaving")
        ...
        # Needed only when gradients are finalized in M-Core
        print_datetime("checking if gradients are finalized in M-core")
        if config.finalize_model_grads_func is not None and not forward_only:
            embedding_module = clear_embedding_activation_buffer(config, model)
        ...
        # Disable async grad reductions
        print_datetime("disabling async grad reductions")
        no_sync_func = config.no_sync_func
        if no_sync_func is None:
            no_sync_func = contextlib.nullcontext
        no_sync_context = None
        ...
        print_datetime("computing num warmup microbatches")
        ...
        print_datetime("getting tensor shapes")
        max_outstanding_backprops = None
        if config.num_microbatches_with_partial_activation_checkpoints is not None:
            max_outstanding_backprops = num_warmup_microbatches + 1
        ...
        # Run warmup forward passes.
        print_datetime(f"Starting warmup forward passes")
        
        mpi_rank = torch.distributed.get_rank()
    
        print(f"[MPI Rank {mpi_rank}, Pipeline Rank {rank}] Starting pipeline execution")
        ...
        for i in range(num_warmup_microbatches):
            ...
            if max_outstanding_backprops is not None:
                ...
                print_datetime(f"Checkpoint decision for batch {i}")
            else:
                checkpoint_activations_microbatch = None
                print(f"[MPI Rank {mpi_rank}, Pipeline Rank {rank}] No checkpointing for batch {i}")

            print(f"[MPI Rank {mpi_rank}, Pipeline Rank {rank}] receive forward for batch {i}")
            input_tensor = recv_forward(recv_tensor_shapes, config)
            print_datetime(f"Forward Step for batch {i}")
            output_tensor, num_tokens = forward_step(
                forward_step_func,
                data_iterator,
                model,
            ...
            print_datetime(f"Sending forward for batch {i}")
        ...
        print_datetime(f"Starting 1F1B steady state")
        ...
        print_datetime('running cooldown backward passes')
        if not forward_only:
            for i in range(num_warmup_microbatches):
        ...
        print_datetime("finishing forward_backward_pipelining_without_interleaving")
        if config.timers is not None:
            config.timers('forward-backward').stop()
    except Exception as e:
        print(f"[Rank {rank}] ERROR in pipeline execution: {str(e)}")
        raise
