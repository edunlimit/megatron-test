...
def check_hf_mg_forward(hfmodel, mgmodel, mgargs):
    ...
    with torch.inference_mode():
        ...
        torch.cuda.synchronize()        # Make sure all CUDA operations are finished
    
        # Optional: Print memory status
        print(f"Memory after HF cleanup: {torch.cuda.memory_allocated()/1e9:.2f}GB")
        print(f"Memory cached: {torch.cuda.memory_reserved()/1e9:.2f}GB")
    
        # Add small delay to ensure memory is freed
        import time
        time.sleep(1)
